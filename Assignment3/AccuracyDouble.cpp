...
int main() {
    double a, b, ab, ab1, error;
    string aStr, bStr, answer;
    int t;
    ofstream file;
    file.open("accuracyDouble.dat");
    cin >> t;
    cout << setw(20) << "num1" << setw(20) << "num2" << setw(35) << "Algo answer" << setw(20) << " '*' op. answer"<< setw(20) << "Absolute Error" << endl;
    ...
    while(t--) {
        ...
        numParser >> a >> b;
        //cout << "a " << a << " b " << b << endl;
        answer = multiply(aStr,bStr);
        stringstream ansParser;
        ansParser << answer;
        ansParser >> ab;
        ab1 = a*b;
        error = abs(ab1-ab);
        cout << setw(20) << aStr << setw(20) << bStr << setw(35) << answer  << setw(20)  << ab1  << setw(20) << error << endl;
        file << ab << '\t' << error << endl;
    }
    file.close();
}